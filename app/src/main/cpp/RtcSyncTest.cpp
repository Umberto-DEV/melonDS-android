...
// A one-second-behind clock, at the default threshold, should still advance: the option's
// whole point is to catch up a lagging clock, and the default threshold is 1 second.
...
// re-stamped every single frame when the two clocks are already effectively in sync.
